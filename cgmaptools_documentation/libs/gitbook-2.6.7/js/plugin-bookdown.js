require(["gitbook", "lodash", "jQuery"], function(gitbook, _, $) {
  ...
  gitbook.events.bind("start", function(e, config) {
    ...
    if (toc && toc.scroll_highlight !== false) (function() {
      ...
      if (rect.top >= ht || rect.top <= 0="" ||="" rect.bottom="" <="0)" {="" summary.scrolltop(li[0].offsettop);="" }="" current="" chapter="" toc="" items="" var="" +="" href="" '"]').parent('li.chapter'),="" m="items.length;" if="" (m="==" 0)="" return;="" all="" section="" titles="" on="" page="" hs="bookInner.find('.page-inner').find('h1,h2,h3')," n="hs.length," ts="hs.map(function(i," el)="" return="" $(el).text();="" });="" (n="==" scrollhandler="function(e)" ht="$(window).height();" cleartimeout($.data(this,="" 'scrolltimer'));="" $.data(this,="" 'scrolltimer',="" settimeout(function()="" find="" the="" first="" visible="" title="" in="" viewport="" for="" (var="" i="0;" n;="" i++)="" rect="hs[i].getBoundingClientRect();" (rect.top="">= 0 && rect.bottom <= ht)="" break;="" }="" if="" (i="==" n)="" return;="" items.removeclass('active');="" for="" (var="" j="0;" <="" m;="" j++)="" {="" (items.eq(j).children('a').first().text()="==" ts[i])="" (j="==" m)="" highlight="" the="" chapter="" title="" search="" bottom-up="" a="" visible="" toc="" item="" to="" highlight;="" an="" is="" hidden,="" we="" check="" its="" parent="" visible,="" and="" so="" on="" while=""> 0 && items.eq(j).is(':hidden')) j--;
